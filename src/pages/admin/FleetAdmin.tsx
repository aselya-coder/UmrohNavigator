...
type Fleet = { id: number; name: string; type: string; capacity: number; price: string; price_per_hour?: string; image: string };

const FleetAdmin = () => {
  const [items, setItems] = useState<Fleet[]>([]);
  const [open, setOpen] = useState(false);
  const [editing, setEditing] = useState<Fleet | null>(null);
  const [form, setForm] = useState<Omit<Fleet, "id">>({ name: "", type: "", capacity: 0, price: "", price_per_hour: "", image: "" });
  ...
  const save = async () => {
    ...
    setOpen(false);
      setEditing(null);
      setForm({ name: "", type: "", capacity: 0, price: "", price_per_hour: "", image: "" });
      setFile(null);
      await load();
    };
    ...
    return (
      <div className="space-y-4">
        <div className="flex justify-between items-center">
          ...
          <Button className="bg-green-600 hover:bg-green-700 text-white" onClick={() => { setEditing(null); setForm({ name: "", type: "", capacity: 0, price: "", price_per_hour: "", image: "" }); setFile(null); setOpen(true); }}>Tambah</Button>
        </div>
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="bg-gray-50">
              ...
              <th className="px-3 py-2 text-left">Price/hr</th>
          ...
          <tbody>
            {items.map((f) => (
              <tr key={f.id} className="border-t">
                ...
                <td className="px-3 py-2">{f.price_per_hour || "—"}</td>
                <td className="px-3 py-2">{f.image}</td>
                <td className="px-3 py-2 space-x-2">
                  <Button variant="outline" className="border-yellow-500 text-yellow-700" onClick={() => { setEditing(f); setForm({ name: f.name, type: f.type, capacity: f.capacity, price: f.price, price_per_hour: f.price_per_hour || "", image: f.image }); setFile(null); setOpen(true); }}>Edit</Button>
      ...
      <Dialog open={open} onOpenChange={setOpen}>
        <DialogContent>
          ...
          <div className="space-y-4">
            ...
            <div className="space-y-2">
              <Label>Type</Label>
              <Input value={form.type} onChange={(e) => setForm((f) => ({ ...f, type: e.target.value }))} placeholder="e.g. Sedan, Van, SUV" />
            </div>
            <div className="space-y-2">
            ...
            <div className="space-y-2">
              ...
              <Input value={form.price_per_hour} onChange={(e) => setForm((f) => ({ ...f, price_per_hour: e.target.value }))} placeholder="e.g. SAR 150" />
            </div>
            <div className="space-y-2">
              <Label>Image</Label>
